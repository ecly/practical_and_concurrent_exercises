...
public class TestCache {
	public static void main(String[] args) throws InterruptedException {
		...
		Memoizer1 m1 = new Memoizer1<Long, long[]>(f1);
		Mark7(m1.getClass().toString(), i -> {
			exerciseFactorizer(m1);
			return i;
		});
		Factorizer f2 = new Factorizer();
		Memoizer2 m2 = new Memoizer2<Long, long[]>(f2);
		Mark7(m2.getClass().toString(), i -> {
			exerciseFactorizer(m2);
			return i;
		});
		Factorizer f3 = new Factorizer();
		Memoizer3 m3 = new Memoizer3<Long, long[]>(f3);
		Mark7(m3.getClass().toString(), i -> {
			exerciseFactorizer(m3);
			return i;
		});
		Factorizer f4 = new Factorizer();
		Memoizer4 m4 = new Memoizer4<Long, long[]>(f4);
		Mark7(m4.getClass().toString(), i -> {
			exerciseFactorizer(m4);
			return i;
		});
		Factorizer f5 = new Factorizer();
		Memoizer5 m5 = new Memoizer5<Long, long[]>(f5);
		Mark7(m5.getClass().toString(), i -> {
			exerciseFactorizer(m5);
			return i;
		});	
		Factorizer f = new Factorizer();
		Memoizer m = new Memoizer<Long, long[]>(f);
		Mark7(m.getClass().toString(), i -> {
	...
	private static void Mark7(String msg, IntToDoubleFunction f) {
		...
		System.out.printf("%-25s,%15.1f us\n", msg, mean);
	...
	private static void exerciseFactorizer(Computable<Long, long[]> f) {
		...
		for(int i = 0; i < threadCount; i++){
			...
			final long from2 = start+range+h*range/4;
			final long to2 = from2+range;
			threads[i]=  new Thread(() -> { 
				for (long p=from1; p<to1; p++){
					try{
						f.compute(p);
					} catch (Exception e){}
				}

				for (long q=from2; q<to2; q++){
					try{
						f.compute(q);
					} catch (Exception e){}
				}
...
class Memoizer <A, V> implements Computable<A, V> {
	...
	public V compute(final A arg) throws InterruptedException {
		V v = cache.computeIfAbsent(arg, (A argv) -> {
			try {
				return c.compute(argv);
