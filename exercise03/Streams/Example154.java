...
class Example154 {
    public static void main(String[] args) {
        ...
        FunList<Integer> list16 = list7.scan((y, x) -> y + x);
...
class FunList<T> {
    ...
    public FunList<T> scan(BinaryOperator<T> f){
        Node<T> x = first.next;
        Node<T> y = new Node<T>(first.item, null);
        FunList<T> list = new FunList<T>(y);
        while (x != null){
            y = new Node<T>(f.apply(y.item, x.item), null);
            list = list.append(new FunList<T>(y));
            x = x.next;
        }
        return list;
    }
